...
if [ -f "/tmp/todolist/frontend.tar.gz" ]; then
    echo "Found file /tmp/todolist/frontend.tar.gz in the target server. Will start the deploy..."
    ...
    mv /tmp/todolist/frontend.tar.gz /www/wwwroot/todolist/frontend
    cd /www/wwwroot/todolist/frontend
    echo "Exploding the tar file..."
    sudo tar xvzf frontend.tar.gz --strip-components 1
    sudo rm -f frontend.tar.gz
    ...
    echo "File /tmp/todolist/frontend.tar.gz does not exist. Deploy didn't succeed. Old application version is still running!"
fi
ENDSSH
