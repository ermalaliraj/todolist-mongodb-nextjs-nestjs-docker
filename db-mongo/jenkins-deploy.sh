...
DEPLOY_DIR=/www/wwwroot/todolist/db-mongo
ZIP_NAME=db-mongo.tar.gz
...
set -e
if [ -f "/tmp/todolist/db-mongo.tar.gz" ]; then
    echo "Found file /tmp/todolist/db-mongo.tar.gz in the target server. Will start the deploy..."
    sudo rm -rf /www/wwwroot/todolist/db-mongo
    sudo mkdir -p /www/wwwroot/todolist/db-mongo
    sudo chown -R jenkins:jenkins /www/wwwroot/todolist/db-mongo
    mv /tmp/todolist/db-mongo.tar.gz /www/wwwroot/todolist/db-mongo
    cd /www/wwwroot/todolist/db-mongo
