...
pwd
SRC_DIR=elk-elasticsearch
STAGE_DIR=/tmp/todolist
DEPLOY_DIR=/www/wwwroot/todolist/elk-elasticsearch
ZIP_NAME=elk-elasticsearch.tar.gz
SCRIPT_INSTALL=jenkins-deploy.sh
...
set -e
if [ -f "/tmp/todolist/elk-elasticsearch.tar.gz" ]; then
    echo "Found file /tmp/todolist/elk-elasticsearch.tar.gz in the target server. Will start the deploy..."
    sudo rm -rf /www/wwwroot/todolist/elk-elasticsearch
    sudo mkdir -p /www/wwwroot/todolist/elk-elasticsearch
    sudo chown -R jenkins:jenkins /www/wwwroot/todolist/elk-elasticsearch
    mv /tmp/todolist/elk-elasticsearch.tar.gz /www/wwwroot/todolist/elk-elasticsearch
    cd /www/wwwroot/todolist/elk-elasticsearch
    echo "Exploding the tar file in /www/wwwroot/todolist/elk-elasticsearch ..."
    sudo tar xvzf elk-elasticsearch.tar.gz --strip-components 1
    sudo rm -f elk-elasticsearch.tar.gz
    chmod o+x docker-deploy.sh
    echo "Starting docker deploy..."
    sudo bash docker-deploy.sh
    echo "✅ elk-elasticsearch deployed successfully."
else
    echo "File /tmp/todolist/elk-elasticsearch.tar.gz does not exist. Deploy didn't succeed. Old application version is still running!"
fi
ENDSSH