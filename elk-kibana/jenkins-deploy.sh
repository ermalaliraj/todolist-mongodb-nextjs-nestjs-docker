...
DEPLOY_DIR=/www/wwwroot/todolist/elk-kibana
ZIP_NAME=elk-kibana.tar.gz
...
set -e
if [ -f "/tmp/todolist/elk-kibana.tar.gz" ]; then
    echo "Found file /tmp/todolist/elk-kibana.tar.gz in the target server. Will start the deploy..."
    sudo rm -rf /www/wwwroot/todolist/elk-kibana
    sudo mkdir -p /www/wwwroot/todolist/elk-kibana
    sudo chown -R jenkins:jenkins /www/wwwroot/todolist/elk-kibana
    mv /tmp/todolist/elk-kibana.tar.gz /www/wwwroot/todolist/elk-kibana
    cd /www/wwwroot/todolist/elk-kibana
    echo "Exploding the tar file in /www/wwwroot/todolist/elk-kibana ..."
    sudo tar xvzf elk-kibana.tar.gz --strip-components 1
    sudo rm -f elk-kibana.tar.gz
    chmod o+x docker-deploy.sh
    echo "Starting docker deploy..."
    sudo bash docker-deploy.sh
    echo "✅ elk-kibana deployed successfully."
else
    echo "File /tmp/todolist/elk-kibana.tar.gz does not exist. Deploy didn't succeed. Old application version is still running!"
fi
ENDSSH